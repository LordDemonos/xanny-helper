...
exports.OffnightFileManager = void 0;
const promises_1 = __importDefault(require("fs/promises"));
const path_1 = __importDefault(require("path"));
const logger_1 = require("../../utils/logger");
const offnightPatternMatcher_1 = require("./offnightPatternMatcher");
const index_1 = require("../../index");
class OffnightFileManager {
    constructor(filePath, cacheManager) {
    ...
    formatEventForFile(event) {
        return (0, offnightPatternMatcher_1.formatOffnightEvent)(event);
    }
    /**
     * Parses date from a line in offnight.txt format
     */
    parseDateFromLine(line) {
        ...
        try {
            ...
            for (const line of lines) {
                ...
                if (isManual) {
                    ...
                    logger_1.logger.debug(`Preserving manual entry: "${line}"`);
                }
                else {
                    logger_1.logger.debug(`Removing line: "${line}"`);
                }
            }
            logger_1.logger.info(`Found ${manualEntries.length} true manual entries in offnight.txt`);
            return manualEntries;
        }
        catch (error) {
            if (error.code === 'ENOENT') {
                logger_1.logger.info('No offnight.txt file found');
                return [];
            }
            logger_1.logger.error(`Error reading manual entries: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async cleanupPastBotEvents() {
        try {
            ...
            for (const line of lines) {
                ...
                if (botEventLines.has(line)) {
                    ...
                    if (eventDate && eventDate < currentDate) {
                        ...
                        logger_1.logger.info(`🗑️ Removing past bot event: ${line}`);
                        removedCount++;
                    }
                    else {
                    ...
                    logger_1.logger.info(`📝 Preserving manual entry: ${line}`);
                    cleanedLines.push(line);
                    preservedCount++;
                }
            }
            if (removedCount > 0) {
                ...
                await promises_1.default.writeFile(this.filePath, cleanedContent, 'utf-8');
                logger_1.logger.info(`🧹 Cleaned up ${removedCount} past bot events, preserved ${preservedCount} entries`);
                // Sync to GitHub after cleanup
                const syncSuccess = await this.syncToGitHub();
                if (!syncSuccess) {
                    logger_1.logger.error('❌ GitHub sync failed after cleanup');
                }
            }
            else {
                logger_1.logger.info(`📋 No past bot events found to clean up`);
            }
            return { removedCount, preservedCount };
        }
        catch (error) {
            if (error.code === 'ENOENT') {
                logger_1.logger.info('📄 No offnight.txt file found for cleanup');
                return { removedCount: 0, preservedCount: 0 };
            }
            logger_1.logger.error(`❌ Error cleaning up past bot events: ${error.message}`);
            throw error;
        }
    }
    /**
     ...
     */
    generateFileContent(events, manualEntries) {
     ...
     */
    async writeOffnightFile(content) {
        try {
            // Ensure directory exists
            const dir = path_1.default.dirname(this.filePath);
            await promises_1.default.mkdir(dir, { recursive: true });
            await promises_1.default.writeFile(this.filePath, content, 'utf-8');
            logger_1.logger.info(`📄 Successfully wrote updated content to offnight.txt`);
        }
        catch (error) {
            logger_1.logger.error(`❌ Error writing offnight.txt: ${error instanceof Error ? error.message : error}`);
            throw error;
        }
    }
    /**
     ...
     */
    hasEventsChanged(newEvents) {
        const cached = this.cacheManager.getOffnightSchedule();
        if (!cached) {
            logger_1.logger.info('📋 No cached offnight schedule found, will update');
            return true;
        }
        ...
        for (const threadId of Object.keys(newThreadIds)) {
            if (!cachedThreadIds[threadId]) {
                logger_1.logger.info(`📋 New thread found: ${threadId}`);
                return true;
            }
        }
        ...
        for (const threadId of Object.keys(cachedThreadIds)) {
            if (!newThreadIds[threadId]) {
                logger_1.logger.info(`📋 Thread removed: ${threadId}`);
                return true;
            }
        }
        ...
        for (const threadId of Object.keys(newThreadIds)) {
            if (cachedThreadIds[threadId]) {
                ...
                if (JSON.stringify(newDates) !== JSON.stringify(cachedDates)) {
                    logger_1.logger.info(`📋 Dates changed for thread: ${threadId}`);
                    return true;
                }
            }
        }
        logger_1.logger.info('📋 No changes detected in offnight events');
        return false;
    }
    /**
     * Updates the cache with new events
     */
    updateCache(newEvents, manualEntries) {
        ...
        // Update cache
        this.cacheManager.updateOffnightSchedule((0, offnightPatternMatcher_1.formatOffnightEvents)(newEvents), Date.now(), threadIds, manualEntryContent);
        logger_1.logger.info(`💾 Updated offnight cache with ${newEvents.length} events from ${Object.keys(threadIds).length} threads and ${manualEntries.length} manual entries`);
    }
    /**
    ...
    async syncToGitHub() {
        try {
            ...
            if (!githubPath) {
                logger_1.logger.error('OFFNIGHT_FILE_PATH is not set in environment variables');
                return false;
            }
            // Read the latest local file content
            const content = await promises_1.default.readFile(this.filePath, 'utf-8');
            logger_1.logger.info(`🔗 Syncing offnight.txt to GitHub at ${githubPath}...`);
            // Use the global updateGithubFile utility (assume imported)
            const success = await (0, index_1.updateGithubFile)(githubPath, content, false);
            if (success) {
                logger_1.logger.info('✅ Successfully synced offnight.txt to GitHub');
            }
            else {
                logger_1.logger.error('❌ Failed to sync offnight.txt to GitHub');
            }
            return success;
        }
        catch (error) {
            logger_1.logger.error(`❌ Error syncing offnight.txt to GitHub: ${error instanceof Error ? error.message : error}`);
            return false;
        }
    }
    /**
    ...
    async readFile() {
        try {
            ...
            logger_1.logger.error(`❌ Error reading offnight.txt: ${error.message}`);
            throw error;
        }
    }
}
exports.OffnightFileManager = OffnightFileManager;
//# sourceMappingURL=offnightFileManager.js.map