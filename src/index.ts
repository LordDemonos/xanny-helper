...
(async () => {
  if (process.argv.includes('--clear-offnight-cache')) {
    ...
    const CACHE_FILE_PATH = process.env.CACHE_FILE_PATH || path.join(__dirname, '..', 'cache', 'content-cache.json');
...
import { processRaidScheduleMessage } from './modules/raid/raidPatternMatcher';
...
function setupMessageHandlers(
  ...
  client.on('messageUpdate', async (oldMessage, newMessage) => {
    ...
    if (oldMessage.partial || newMessage.partial || oldMessage.author?.bot || newMessage.author?.bot) return;

    // Handle edited raid schedule messages
    if (newMessage.channel.id === raidChannel.id) {
      log(`\n📝 Raid message edited in #${(newMessage.channel as TextChannel).name} by ${newMessage.author.tag}`);
      try {
    ...
    if (newMessage.channel.id === inventoryChannel.id) {
      log(`\n📝 Inventory message edited in #${(newMessage.channel as TextChannel).name} by ${newMessage.author.tag}`);
