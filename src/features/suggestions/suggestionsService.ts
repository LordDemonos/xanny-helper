...
export class SuggestionsService {
  ...
  private readonly SUGGESTION_PREFIX = 'Feedback & Suggestions Bot:';

  constructor(
    client: Client,
    channelId: string,
    credentials: { keyFile: string; scopes: string[] },
    ...
    this.channel = client.channels.cache.get(channelId) as TextChannel;
  ...
  private startChecking(): void {
    ...
    this.checkInterval = setInterval(async () => {
      try {
        ...
        logger.error('Error checking for new suggestions:', error);
      }
    }, 15 * 60 * 1000); // 15 minutes in milliseconds

    // Do an initial check
    this.checkForNewSuggestions().catch(error => {
      logger.error('Error during initial suggestions check:', error);
    });
  }
  ...
  private async checkForNewSuggestions(): Promise<void> {
    try {
      ...
      const newSuggestions = suggestions.filter(suggestion => 
        !postedSuggestions.has(suggestion.timestamp)
      );
      
      if (newSuggestions.length === 0) {
        logger.info('No new suggestions to post');
        return;
      }
      ...
      for (const suggestion of newSuggestions) {
        ...
        if (filterReason) {
          logger.warn(`Suggestion from [${suggestion.timestamp}] filtered: ${filterReason}`);
          continue;
        }
        
        const message = this.formatSuggestion(suggestion);
        await this.channel.send(message);
        logger.info(`Posted new suggestion from [${suggestion.timestamp}] to Discord`);
        
        // Rate limit: wait 15 seconds before next post
        await new Promise(resolve => setTimeout(resolve, 15000));
      }
      
      logger.info(`Finished posting ${newSuggestions.length} new suggestion(s) to Discord`);
    } catch (error) {
      logger.error('Error checking for new suggestions:', error);
      throw error;
    }
  ...
  private async readSuggestions(): Promise<Suggestion[]> {
    ...
    try {
        logger.info(`Reading suggestions from Google Sheet: ${this.spreadsheetId} (attempt ${attempt}/${maxRetries})`);
      ...
      const rows = response.data.values as GoogleSheetsRow[];
      if (!rows || rows.length === 0) {
        logger.info('No suggestions found in sheet');
        return [];
      }

      logger.info(`Found ${rows.length - 1} suggestions in sheet`);
      return rows.slice(1).map((row: GoogleSheetsRow): Suggestion => ({
        timestamp: row[0],
        suggestion: row[1],
        imageUrl: row[2] || undefined,
      }));
    } catch (error) {
        logger.error(`Error reading suggestions (attempt ${attempt}/${maxRetries}):`, error);
        
        // Add more detailed error information for debugging
        if (error instanceof Error) {
          if (error.message.includes('ENOTFOUND') || error.message.includes('EHOSTUNREACH')) {
            logger.warn(`Network connectivity issue detected: ${error.message}`);
          } else if (error.message.includes('timeout')) {
            logger.warn(`Request timeout detected: ${error.message}`);
          } else if (error.message.includes('ECONNRESET')) {
            logger.warn(`Connection reset detected: ${error.message}`);
          }
        }
  ...
  private async getPostedSuggestions(): Promise<Set<string>> {
    try {
      logger.info(`Fetching previously posted suggestions from Discord channel: ${this.channel.id}`);
      const messages = await this.channel.messages.fetch({ limit: 100 });
      
      const postedSuggestions = new Set<string>();
      let count = 0;
      ...
      logger.info(`Found ${count} previously posted suggestions in Discord channel`);
      return postedSuggestions;
    } catch (error) {
      logger.error('Error fetching posted suggestions from Discord:', error);
      throw error;
    }
  }

  private formatSuggestion(suggestion: Suggestion): string {
  ...
  private filterSuggestion(suggestion: Suggestion): string | null {
