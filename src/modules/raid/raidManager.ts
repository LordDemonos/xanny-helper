...
export class RaidManager {
  ...
  async processRaidSchedule(): Promise<void> {
    try {
      ...
      // Check and update calendar if enabled
    ...
    try {
      ...
      if (!cachedSchedule || cachedSchedule.content !== content) {
        ...
        if (result[0].success) {
          ...
          // Only update calendar after successful schedule processing if enabled
          if (this.calendarEnabled) {
            try {
        ...
        // Even if content hasn't changed, still check calendar for missing events if enabled
        if (this.calendarEnabled) {
          try {
          ...
          logger.info('📅 Calendar functions are disabled - skipping Google Calendar check');
        }
      }
    } catch (error) {
