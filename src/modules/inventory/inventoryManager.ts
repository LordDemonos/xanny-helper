...
export class InventoryManager {
  ...
  constructor(
    ...
    this.silentMode = process.env.SILENT_INVENTORY_UPDATES === 'true';
    
    if (this.silentMode) {
      logger.info('🔇 Inventory notifications are disabled (SILENT_INVENTORY_UPDATES=true)');
    }
  }
    ...
    try {
      ...
      for (const [cacheKey, cachedData] of Object.entries(cache.inventoryFiles)) {
        ...
        if (fileName === 'Fggems-Inventory.txt' ||
            ...
            fileName === 'Fgspellsdump-Inventory.txt') {
          operations.push({
            path: githubPath,
          ...
          if (result.success) {
            ...
            if (!cachedData) {
              if (!this.silentMode) {
                ...
                logger.info(`🔇 Silent mode: Skipping Discord notification for ${fileName}`);
              }
            } else {
