...
export interface DiscordEventConfig {
  ...
  offnightFileOperationsEnabled?: boolean; // Add offnight file operations flag
}
...
class DiscordEventManager {
  ...
  private offnightFileOperationsEnabled: boolean;
  ...
  constructor(config: DiscordEventConfig) {
    ...
    this.offnightFileOperationsEnabled = config.offnightFileOperationsEnabled ?? true;
    // logger.info(`🔧 Debug: DiscordEventManager constructor - offnightFileOperationsEnabled = ${this.offnightFileOperationsEnabled}`);
  }
  ...
  private async parseOffnightEvents(): Promise<ParsedEvent[]> {
    // logger.info(`🔧 Debug: offnightFileOperationsEnabled = ${this.offnightFileOperationsEnabled}`);
    if (!this.offnightFileOperationsEnabled) {
      logger.info('📄 Offnight file operations are disabled - skipping offnight events for Discord events');
      return [];
    }
