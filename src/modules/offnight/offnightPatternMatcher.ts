...
export interface ParsedOffnightEvent {
  ...
  date: Date;
  time: string;
  timeRange?: string; // For events with start and end times
  isRecurring: boolean;
  timezone: string;
  originalThreadId?: string;
  threadCreatedAt?: Date; // When the thread was created
  threadCreator?: string; // Name of the person who created the thread
}
 ...
 */
function extractDay(title: string): string | null {
 ...
 */
function extractTimezone(title: string): string {
 ...
 */
function convertToEST(timeStr: string, fromTimezone: string): string {
 ...
 */
function parseTime(title: string, eventDate?: Date): { time: string; timeRange?: string } {
  ...
  if (isBeforeRaid || isAfterRaid) {
    if (eventDate) {
      ...
      if (isBeforeRaid) return { time: '7:00 PM EST' };
      if (isAfterRaid) return { time: '11:30 PM EST' };
    }
  }
 ...
 */
function parseDatesWithContext(title: string, threadCreatedAt?: Date): Date[] {
  const dates: Date[] = [];
  const now = new Date();
 ...
 */
function inferDayFromContext(title: string, threadCreatedAt?: Date): string | null {
 ...
 */
function findNextOccurrence(dayName: string, referenceDate: Date, maxWeeks: number = 8): Date | null {
 ...
 */
export function parseOffnightTitle(title: string, threadCreatedAt?: Date): ParsedOffnightEvent | null {
  try {
    ...
    logger.error(`Error parsing offnight title "${title}": ${error instanceof Error ? error.message : error}`);
    return null;
  }
}
 ...
 */
export function generateRecurringEvents(event: ParsedOffnightEvent): ParsedOffnightEvent[] {
 ...
 */
export function processOffnightThreads(threads: { name: string; id: string }[]): ParsedOffnightEvent[] {
  const validEvents: ParsedOffnightEvent[] = [];
  const currentDate = new Date();

  logger.info(`Processing ${threads.length} offnight threads`);
  ...
  for (const thread of threads) {
    ...
    if (parsedEvent.date < currentDate && !parsedEvent.isRecurring) {
      logger.info(`Skipping past event: ${thread.name}`);
      continue;
    }
  ...
  logger.info(`Found ${validEvents.length} valid offnight events`);
  return validEvents;
}
 ...
 */
export function formatOffnightEvent(event: ParsedOffnightEvent): string {
 ...
 */
export function formatOffnightEvents(events: ParsedOffnightEvent[]): string {
  // Sort events by date
 ...
 */
function inferRaidTime(eventDate: Date, isBeforeRaid: boolean, isAfterRaid: boolean): string {
  const dayName = DAYS_OF_WEEK[eventDate.getDay()];
  const raidInfo = RAID_SCHEDULE[dayName as keyof typeof RAID_SCHEDULE];
  
  if (!raidInfo) {
    // No scheduled raid on this day, use default times
    if (isBeforeRaid) return '7:00 PM EST';
    if (isAfterRaid) return '11:30 PM EST';
    return '8:00 PM EST';
  }
