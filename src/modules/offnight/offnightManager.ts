...
export class OffnightManager {
  ...
  constructor(
    ...
    private readonly fileOperationsEnabled: boolean = true,
    private readonly calendarEnabled: boolean = true
  ) {
  ...
  async findLatestOffnightThreads(channel: TextChannel): Promise<void> {
    try {
      ...
      const INACTIVITY_THRESHOLD_DAYS = 7; // Consider threads inactive after 7 days of no activity
      ...
      for (const thread of threads) {
        if (thread.createdAt !== null) {
          ...
          // Check for thread inactivity based on last message
          let isInactive = false;
          if (thread.lastMessageId) {
            try {
              const lastMessage = await thread.messages.fetch(thread.lastMessageId);
              const daysSinceLastActivity = (Date.now() - lastMessage.createdTimestamp) / (1000 * 60 * 60 * 24);
              
              if (daysSinceLastActivity > INACTIVITY_THRESHOLD_DAYS) {
                logger.info(`⏰ Thread "${thread.name}" is inactive (${Math.floor(daysSinceLastActivity)} days since last message) - skipping`);
                skippedInactiveCount++;
                isInactive = true;
              }
            } catch (error) {
              // If we can't fetch the last message, assume the thread is inactive
              logger.warn(`⚠️ Could not fetch last message for thread "${thread.name}": ${error instanceof Error ? error.message : error} - assuming inactive`);
              skippedInactiveCount++;
              isInactive = true;
            }
          } else {
            // No last message ID, check if thread is old enough to be considered inactive
            if (thread.createdTimestamp) {
              const daysSinceCreation = (Date.now() - thread.createdTimestamp) / (1000 * 60 * 60 * 24);
              if (daysSinceCreation > INACTIVITY_THRESHOLD_DAYS) {
                logger.info(`⏰ Thread "${thread.name}" has no messages and is ${Math.floor(daysSinceCreation)} days old - skipping`);
                skippedInactiveCount++;
                isInactive = true;
              }
            } else {
              // No creation timestamp, assume inactive
              logger.warn(`⚠️ Thread "${thread.name}" has no creation timestamp - assuming inactive`);
              skippedInactiveCount++;
              isInactive = true;
            }
          }
      ...
      if (skippedInactiveCount > 0) {
        logger.warn(`⏰ Skipped ${skippedInactiveCount} inactive threads (no activity for ${INACTIVITY_THRESHOLD_DAYS}+ days)`);
      }
  ...
  async processAndUpdateEvents(events: ParsedOffnightEvent[]): Promise<boolean> {
    try {
      if (!this.fileOperationsEnabled) {
        logger.info('📄 File operations are disabled - skipping offnight.txt file processing');
        logger.info(`📊 Found ${events.length} offnight events but not updating file`);
        return false;
      }
      ...
      // 9. Update Google Calendar if enabled and calendar ID is provided
      if (this.calendarEnabled && this.googleCalendarId) {
        try {
      ...
      } else if (!this.calendarEnabled) {
        logger.info('📅 Calendar functions are disabled - skipping Google Calendar update');
      }
