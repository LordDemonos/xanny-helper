...
const monsterMappings: Record<string, string> = {
 ...
 */
function findFirstMonsterMatch(text: string): string | null {
 ...
 */
export function getEventImageName(eventTitle: string, eventDescription?: string, eventType: 'raid' | 'offnight' = 'raid'): string {
 ...
 */
export function getImageUrl(imageName: string, githubRepo: string, branch: string = 'main'): string {
 ...
 */
export function testEventImage(eventTitle: string, eventDescription?: string, eventType: 'raid' | 'offnight' = 'raid'): {
  imageName: string;
  matchedText?: string;
} {
  const searchText = `${eventTitle} ${eventDescription || ''}`;
 ...
 */
export async function githubImageToDataURI(imageUrl: string, cacheManager: any): Promise<string | null> {
  try {
    // Check cache first
    ...
    if (cached && cached.data && cached.timestamp) {
      ...
      if (ageInDays < 30) {
        logger.info(`Using cached data URI for image: ${imageUrl}`);
        return cached.data;
      } else {
        logger.info(`Cache expired for image: ${imageUrl}, re-fetching...`);
      }
    }

    // Fetch the image from GitHub
    logger.info(`Fetching image from GitHub: ${imageUrl}`);
    ...
    if (uint8Array.length > 10 * 1024 * 1024) {
      logger.error(`Image too large: ${imageUrl} (${uint8Array.length} bytes, max 10MB)`);
      return null;
    }
    ...
    logger.info(`Successfully converted image to data URI: ${imageUrl} (${uint8Array.length} bytes, ${mimeType})`);
    return dataUri;

  } catch (error) {
    logger.error(`Error converting image to data URI: ${imageUrl}`, error);
    return null;
  }
} 